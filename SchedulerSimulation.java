...
class Process implements Runnable {
  ...
  // feature3 
    public void setLastReadyTime(int time) {
...
public class SchedulerSimulation {
    ...
    public static void main(String[] args) {
        ...
        int studentID = 445052190; // ← CHANGE THIS TO YOUR ACTUAL STUDENT ID
        ...
        while (!processQueue.isEmpty()) {
            ...
            System.out.println(Colors.BOLD + Colors.MAGENTA + "┌─ Ready Queue "
                    + new String(new char[65]).replace("\0", "─") + Colors.RESET);
            ...
            System.out.println(Colors.BOLD + Colors.MAGENTA + "└" + new String(new char[79]).replace("\0", "─")
                    + Colors.RESET + "\n");
            ...
            if (!process.isFinished()) {
                ...
                if (!processQueue.isEmpty()) {
                    //feature3
                    process.setLastReadyTime((int) System.currentTimeMillis());

                    // Re-enqueue the process to give it another chance to run in the next round
                    addProcessToQueue(process, processQueue, processMap);
                } else {
                    // If this is the last process in the queue, runs it to completion
